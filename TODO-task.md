...
> 基于 openspec `amount-validation-word-export` change 新增，于 2026-06-12 记录。于 2026-06-13 全部完成。

## ✅ Task 11: 上传支付记录时校验金额一致性
- **Commit**: `481a3cd`
- **实现**: `AmountValidationService` OCR 识别上传文件中的金额，与记录金额比对；不一致显示 SnackBar 提示但仍保存
- **测试**: `amount_validation_service_test.dart` — 金额一致、不一致、无法识别、PDF 文本提取

## ✅ Task 12: 上传发票时校验金额一致性
- **Commit**: `481a3cd`
- **实现**: 同 Task 11，`_uploadInvoice()` 调用 `_warnIfAmountMismatch()` 校验发票金额
- **测试**: 同上，覆盖 PDF/图片发票场景

## ✅ Task 13: 生成月度打印裁剪 Word 文件
- **Commits**: `d6f11ee`, `ded937a`, `7aa07fd`, `7d10837`
- **实现**: `PrintWordService` 生成 A4 排版 Word（发票全宽、支付记录 1/4 宽 × 1/3 高、一行最多 3 张）；支持 PDF 发票渲染为图片；Word 自动纳入月度 ZIP
- **测试**: `print_word_service_test.dart` — 混合 PDF/图片发票、多支付记录排版
