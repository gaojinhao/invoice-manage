...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Future<void> _recognize() async {
    ...
    try {
      ...
      final result = await ocr.recognizeImage(_image!);
      ocr.dispose();

      _ocrRawText = result.rawText;

      if (result.merchant != null) _merchantCtrl.text = result.merchant!;
      if (result.amount != null) _amountCtrl.text = result.amount!.toStringAsFixed(2);
      if (result.date != null) {
        _dateCtrl.text = '${result.date!.year}-${result.date!.month.toString().padLeft(2, '0')}-${result.date!.day.toString().padLeft(2, '0')}';
      }
    } catch (_) {
      // OCR 失败，进入手动模式
      _manualMode = true;
  ...
  Future<void> _save() async {
    if (_merchantCtrl.text.isEmpty || _amountCtrl.text.isEmpty || _dateCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请填写商户名、金额和日期')),
      );
      return;
    }
    ...
    try {
      ...
      String? receiptPath;
      if (_image != null) {
        receiptPath = await fileService.saveReceiptImage(_image!, date, _merchantCtrl.text);
      }
              ...
              ExpansionTile(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(_ocrRawText!, style: const TextStyle(fontSize: 12)),
                  ),
                ],
              ),
            ],
